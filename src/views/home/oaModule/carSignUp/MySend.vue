        ...
        >
          ...
          >
            <div class="list-item" style="margin: 2px">
              <van-cell
                :value="item.goOutVehicleVO?.state === 4 ? '去登记' : '详情'"
                is-link
...
import { fetchEndSignUpList } from "@/api/outApply";
import {
  DropdownItemOption,
  TagType,
  closeToast,
...
const clickDetail = (item) => {
  router.push(
    `/oa/carSignUp/submit?id=${item.id}&type=back&goOutStatus=${item.goOutVehicleVO?.state}`
  );
};
...
const getList = () => {
  ...
  fetchEndSignUpList({ goOutStatus: selectedMenuValue.value })
    .then((res) => {
