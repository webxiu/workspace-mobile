<template>
  <div>
    <div class="detail-page">
      <van-notice-bar class="user-title" color="#1989fa" background="#ecf9ff" left-icon="info-o"> 【{{ detailInfo.staffName }}】的加班单 </van-notice-bar>
      <div>
        <div class="detail">
          ...
          <div class="des-item">
            <div style="display: flex; align-items: center">
              ...
              <div class="value">{{ detailInfo.staffName }}</div>
            </div>
          </div>
          <div class="des-item">
            <div style="display: flex; align-items: center">
              <div class="label">加班单号</div>
              <div class="value">{{ detailInfo.billNo }}</div>
            </div>
          </div>
          <div class="des-item">
            <div style="display: flex; align-items: center">
              <div class="label">加班类型</div>
              <div class="value">{{ detailInfo.overtimeType }}</div>
            </div>
          </div>
          <div class="des-item">
            <div style="display: flex; align-items: center">
              <div class="label">加班缘由</div>
              <div class="value">
                {{ detailInfo.remark || "无" }}
              </div>
            </div>
    ...
    <van-tabbar @change="changeBottomBar">
      ...
      <!-- 审核节点详情  start-->
      <NodeDetailModal ref="nodeRef" :detailInfo="detailInfo" billType="overTime" />
      <!-- 审核节点详情  end -->
...
import { deleteLeaveList, deleteOverTimeList, getOverTimeDetail, revokeLeaveList, revokeOverTimeList } from "@/api/oaModule";
...
const handleAction = (actionType) => {
  ...
  showConfirmDialog({
    message: `您确定要${actionType === "del" ? "删除" : actionType === "submit" ? "提交" : "撤销"}当前加班单吗`,
