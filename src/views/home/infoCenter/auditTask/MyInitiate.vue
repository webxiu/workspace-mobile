                    ...
                    <div class="flex just-between ui-w-100">
                      <div style="color: #aaa" class="ellipsis">
                        ...
                        <div>
                          ...
                          <span class="content-offset">发起时间：{{ item.startTime }}</span>
                        </div>
                      </div>
...
import { getAuditTask3 } from "@/api/infoCenter";
...
const getList = (v?: any) => {
  ...
  getAuditTask3({ limit: limit.value, page: currentPage.value })
    .then((res) => {
      resultList.value = res.data?.records || [];
    })
    .catch(() => {})
...
const initData = (res) => {
  resultList.value = res.data.records.sort((a, b) => (a.startTime < b.startTime ? 1 : -1));
};
...
const gotoDetail = (type, { billNo, billId, processDefId, processInstId, projectId }) => {
  router.push({
    ...
    query: { billNo, billId, processDefId, processInstId, projectId, tab: 3, type }
  });
};
