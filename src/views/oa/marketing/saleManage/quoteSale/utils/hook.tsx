...
import { quoteSaleList, QuoteApplyListItemType, addQuoteSale, updateQuoteSale, deleteQuoteSale } from "@/api/oaManage/marketing";
...
export const useConfig = () => {
  ...
  const getColumnConfig = async () => {
    let columnData: TableColumnList[] = [
      ...
      { label: "是否翻单", prop: "isRepeatOrder", minWidth: 100 },
  ...
  const onDelete = wrapFn(rowData, () => {
    ...
    showMessageBox(`确认要删除申请单号【${billNo}】吗?`).then(() => {
      deleteQuoteSale({ deleteIds: [rowData.value.id] }).then(({ data }) => {
