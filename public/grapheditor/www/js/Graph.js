...
Graph = function (container, model, renderHint, stylesheet, themes, standalone) {
          ...
          if (!mxEvent.isAltDown(me.getEvent()) && state != null) {
            ...
            if (this.model.isEdge(state.cell)) {
              ...
              if (handler == null || handler.getHandleForEvent(me) == null) {
                ...
                box.grow(mxEvent.isTouchEvent(me.getEvent()) ? mxShape.prototype.svgStrokeTolerance - 1 : (mxShape.prototype.svgStrokeTolerance + 1) / 2);
                ...
                if (this.isTableCell(state.cell) && !this.isCellSelected(state.cell)) {
                  ...
                  if (!this.isCellSelected(table)) {
                    if (
                      (mxUtils.intersects(box, new mxRectangle(state.x, state.y - 2, state.width, 3)) && this.model.getChildAt(table, 0) != row) ||
                      mxUtils.intersects(box, new mxRectangle(state.x, state.y + state.height - 2, state.width, 3)) ||
                      (mxUtils.intersects(box, new mxRectangle(state.x - 2, state.y, 2, state.height)) && this.model.getChildAt(row, 0) != state.cell) ||
  ...
  if (typeof mxVertexHandler !== "undefined") {
    ...
    this.connectionHandler.constraintHandler.isStateIgnored = function (state, source) {
      ...
      return (
        source &&
        (graph.isCellSelected(state.cell) || (graph.isTableRow(state.cell) && graph.selectionCellsHandler.isHandled(graph.model.getParent(state.cell))))
      );
    };
 ...
 */
Graph.fileSupport =
  window.File != null && window.FileReader != null && window.FileList != null && (window.urlParams == null || urlParams["filesupport"] != "0");
...
(function () {
  ...
  Graph.prototype.isFastZoomEnabled = function () {
    return urlParams["zoom"] != "nocss" && !mxClient.NO_FO && !mxClient.IS_EDGE && !this.useCssTransforms && this.isCssTransformsSupported();
  };
...
Graph.prototype.isSplitTarget = function (target, cells, evt) {
  return !this.model.isEdge(cells[0]) && !mxEvent.isAltDown(evt) && !mxEvent.isShiftDown(evt) && mxGraph.prototype.isSplitTarget.apply(this, arguments);
};
...
Graph.prototype.createTable = function (rowCount, colCount, w, h, title, startSize, tableStyle, rowStyle, cellStyle) {
  ...
  cellStyle =
    cellStyle != null
      ? cellStyle
      : "shape=partialRectangle;html=1;whiteSpace=wrap;connectable=0;" + "overflow=hidden;fillColor=none;top=0;left=0;bottom=0;right=0;";
  ...
  (function () {
    ...
    Graph.prototype.insertTextForEvent = function (evt, cell) {
      ...
      if (evt != null && !this.model.isVertex(cell)) {
        ...
        if (
          ...
          (state != null || (mxClient.IS_VML && src == this.view.getCanvas()) || (mxClient.IS_SVG && src == this.view.getCanvas().ownerSVGElement))
        ) {
          if (state == null) {
    ...
    mxCellEditor.prototype.getBackgroundColor = function (state) {
      ...
      if (
        ...
        (mxUtils.getValue(state.style, mxConstants.STYLE_ROTATION, 0) != 0 || mxUtils.getValue(state.style, mxConstants.STYLE_HORIZONTAL, 1) == 0)
      ) {
    ...
    mxVertexHandler.prototype.isParentHighlightVisible = function () {
      return (
        vertexHandlerIsParentHighlightVisible.apply(this, arguments) && !this.graph.isTableCell(this.state.cell) && !this.graph.isTableRow(this.state.cell)
      );
    };
    ...
    mxVertexHandler.prototype.updateHint = function (me) {
      if (this.index != mxEvent.LABEL_HANDLE) {
        ...
        if (this.index == mxEvent.ROTATION_HANDLE) {
          ...
          this.hint.innerHTML =
            formatHintText(this.roundLength(this.bounds.width / s), unit) + " x " + formatHintText(this.roundLength(this.bounds.height / s), unit);
        }
      ...
      mxPanningHandler.prototype.isPanningTrigger = function (me) {
        ...
        return (
          (mxEvent.isLeftMouseButton(evt) &&
            ((this.useLeftButtonForPanning && me.getState() == null) || (mxEvent.isControlDown(evt) && !mxEvent.isShiftDown(evt)))) ||
    ...
    mxEdgeHandler.prototype.createHandleShape = function (index, virtual) {
      ...
      var c =
        index != null && (index == 0 || index >= this.state.absolutePoints.length - 1 || (this.constructor == mxElbowEdgeHandler && index == 2))
