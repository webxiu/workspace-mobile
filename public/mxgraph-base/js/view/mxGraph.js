...
mxGraph.prototype.getCellStyle = function (cell) {
  ...
  // console.log(this.stylesheet.getCellStyle(stylename, style), "解析样式");
  var style = null;
  ...
  if (stylename != null) {
    // console.log(stylename.split(";base64,"), "stylename后缀");
    let arr = stylename.split(";base64,");
    // console.log(stylename, "stylename");
    if (Array.isArray(arr) && arr.length > 1) {
      window.base64Suf = arr[1];
      // console.log(arr[1], "后缀");
      // style = this.postProcessCellStyle(this.stylesheet.getCellStyle(stylename, style));
    }
    // console.log(Object.keys(style), "style");
    style = this.postProcessCellStyle(this.stylesheet.getCellStyle(stylename, style));
  }
...
mxGraph.prototype.postProcessCellStyle = function (style) {
  if (style != null) {
    ...
    if (image != null && image.substring(0, 11) == "data:image/") {
      ...
      // console.log(window.base64Suf?.endsWith(";"), "全局的后缀");
      // let str;
      if (window.base64Suf?.endsWith(";")) {
        window.base64Suf = window.base64Suf.substring(0, window.base64Suf.length - 1);
        style[mxConstants.STYLE_IMAGE] = image + ";base64," + window.base64Suf;
      } else {
        style[mxConstants.STYLE_IMAGE] = image;
      }
    }
...
mxGraph.prototype.cellsAdded = function (cells, parent, index, source, target, absolute, constrain, extend) {
  if (cells != null && parent != null && index != null) {
    ...
    try {
      ...
      this.fireEvent(
        new mxEventObject(mxEvent.CELLS_ADDED, "cells", cells, "parent", parent, "index", index, "source", source, "target", target, "absolute", absolute)
      );
    } finally {
      this.model.endUpdate();
    }
