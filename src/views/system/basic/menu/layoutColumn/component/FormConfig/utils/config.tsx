 ...
 * @Last Modified time: 2024-05-21 11:46:44
 */
...
export const formRules = (name): FormRules => ({
  columns: [
    { required: true, message: "输入内容不能为空", trigger: "blur" },
    {
      message: "输入格式错误",
      trigger: "blur",
      pattern: { name: regExp.nameMap, table: regExp.tableMap }[name]
    }
  ]
});
...
export const formConfigs = (type = "table"): FormConfigItemType[] => {
  const name = type === "table" ? "表名.字段" : "名称.字段";
  const label = type === "table" ? "" : "(名称.字段)";
  const placeholder = `输入格式:
${name}
${name}
${name}
...`;
...
// 格式化配置表单
export const formatConfigs = ({ formData, row }): Ref<FormConfigItemType[]> => {
  const enumList = ref([]);
  ...
  const configFn = (): FormConfigItemType[] => [
    ...
    // ############### 公共 ###############
    {
      ...
      hide: false,
      slots: {
        label: () => <LabelQuestion label="布局" content="每行分为24格, 当配置的表单项超过24格, 则换行显示" />
      },
      render: ({ formModel, row }) => {
        return (
    ...
    // ############### 日期 ###############
    {
      ...
      colProp: { span: 12 },
      slots: {
        label: () => <LabelQuestion label="日期类型" content="数据必须是时间格式(时间戳、日期字符串)" />
      },
      render: ({ formModel, row }) => (
    ...
    // ############### 下拉框 ###############
    {
      ...
      colProp: { span: 12 },
      slots: {
        label: () => <LabelQuestion label="接口地址" content="配置接口地址, 请求方式、名称字段、值字段必填" />
      },
    ...
    },
    {
      ...
      slots: {
        label: () => <LabelQuestion label="枚举字典" content="同时配置接口地址和枚举字典, 使用枚举字典数据" />
      },
      colProp: { span: 12 },
      render: ({ formModel, row }) => {
        return (
    ...
    },
    {
      ...
      hide: false,
      slots: {
        label: () => <LabelQuestion label="样式" content="扩展CSS原生样式(如: font-size: 14px; margin: 0px), 多个样式使用分号(;)隔开" />
      },
      render: ({ formModel, row }) => (
        <el-input
          type="textarea"
