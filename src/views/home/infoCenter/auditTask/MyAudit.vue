    ...
    <div class="list-content">
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        <div class="container-box" v-if="resultList.length">
          <van-list :offset="10" v-model:loading="loading" :immediate-check="false" finish-text="没有更多了" :finished="finished" @load="onLoad">
            <div
                    ...
                    <div class="flex just-between ui-w-100">
                      <div style="color: #aaa" class="ellipsis">
                        ...
                        <div>
                          ...
                          <span class="content-offset">发起时间：{{ item.sendTime }}</span>
                        </div>
                      </div>
...
import { getAuditTask1 } from "@/api/infoCenter";
...
const getList = (v?: any) => {
  ...
  getAuditTask1({ limit: limit.value, page: currentPage.value })
    .then((res) => {
      resultList.value = res.data?.records || [];
    })
    .catch(() => {})
...
const gotoDetail = (type, { billNo, billId, processDefId, processInstId, projectId }) => {
  router.push({
    ...
    query: { billNo, billId, processDefId, processInstId, projectId, tab: 1, type }
  });
};
